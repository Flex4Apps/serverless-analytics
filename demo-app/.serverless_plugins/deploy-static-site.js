...
class ServerlessPlugin {
  ...
  syncDirectory() {
    ...
    const siteBuildDir = this.serverless.variables.service.custom.siteBuildDir;
    // might be better way to get to this data, for now, this seems to work...
    var region = this.serverless.service.provider.region;
    var profile = this.serverless.service.provider.profile;
  ...
  domainInfo() {
    ...
    return provider.request('CloudFormation', 'describeStacks', {
      StackName: stackName
    }, this.options.stage, this.options.region // eslint-disable-line comma-dangle
    ).then((result) => {
